...
#' \deqn{\displaystyle\left\{\begin{matrix} \frac{d}{dt} S(t) &=& f_{\overline M_k} e^{-\delta_V(t-t_k)}-\delta_S S(t) \\ \frac{d}{dt} Ab(t) &=& \theta_k S(t) - \delta_{Ab} Ab(t)\end{matrix}\right.  }
...
#' @param parms named vector of model parameter ; should contain "t_0","t_inj","fM2","theta_1","theta_2","delta_S","delta_Ab","delta_V".
#'
...
Clairon <- function(t,y,parms){
  ...
  if(!setequal(names(parms),c("t_0","t_inj","fM2","theta_1","theta_2","delta_S","delta_Ab","delta_V"))){
    stop()
  }

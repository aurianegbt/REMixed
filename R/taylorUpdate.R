...
#'
#' @details
#' The individual trajectory \eqn{(R_{i})_{i\leq N}} is observed through latent processes, up to a transformation \eqn{s_k}, \eqn{k\leq K}, observed in \eqn{(t_{kij})_{i\leq N,k\leq K,j\leq n_{kij}}} : \deqn{Z_{kij}=\alpha_{k0}+\alpha_{k1} s_k(R_i(t_{kij}))+\varepsilon_{kij}} where \eqn{\varepsilon_k\overset{iid}{\sim} \mathcal N(0,\sigma_k^2)}. The individual trajectory \eqn{(R_i)_{i\leq N}} depends on parameter \eqn{\theta}, in which we add the parameter \eqn{\alpha_0=(\alpha_{k0})_{k\leq K}} to differentiate the regularization parameters \eqn{\alpha_1=(\alpha_k1)_{k\leq K}} from the other. For more details on the model, see the package description.
#' For previous parameter \eqn{\alpha_1^* ,\theta^* }, we have the gradient and hessien of the log-likelihood in $(\theta^*,\alpha_1^*)$ (contains in `ddLL`and `dLL`). Then, we update the \eqn{\alpha_1} parameter, for a fixed penalty parameter \eqn{\lambda} (`lambda`), by optimizing the lasso equation \deqn{\partial_{\alpha_{1k}} LL(\theta^*,\alpha_1)|_{\alpha_1=\alpha_1^*}} The update is given by the formula, denoting \eqn{A=\partial_{\alpha_{1k}} LL(\theta^*,\alpha_1)|_{\alpha_1=\alpha_1^*}} - \sum_{l=1,\neq k}^K} x_{lk}\alpha^*_{1l} + \sum_{c=1}^K x_{kc}\alpha_{1c}^*} :
...
#' with \eqn{-\partial_{\alpha_1}^2 LL(\theta^*,\alpha_1)|_{\alpha_1^*}=-(x_{lc})_{l,c\leq K}}
#'
#' @param alpha previous estimation of the regulatization parameter \eqn{\alpha_1} ;
#' @param lambda penalty parameter ;
#' @param ddLL Hessienne matrix of the log-likelihood in previous estimated \eqn{\theta^*,\alpha_1^*} ;
#' @param dLL Gradient of the log-likelihood in previous estimated \eqn{\theta^*,\alpha_1^*}.
#'

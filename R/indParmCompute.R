...
#' \deqn{ g_k(\Psi_{ki}) = g_k(\Psi_{kpop})+X_i B_l + \eta_{ki}}
#' with \eqn{g_k} the transformation and  \eqn{B_k} the vector of covariates effect associated \eqn{\Psi_k} parameter.
#'
#' @param theta model parameter contain Psi_pop, psi_pop, Beta, beta ; list with name `Psi_pop`, `psi_pop`, `Beta`, `beta` ;
#'
#' - `Psi_pop` named vector of population parameters without r.e ;
#'
#' - `psi_pop` named vector of population parameters with r.e ;
#'
#' - `Beta` named list of vector of covariates effects for  `Psi_pop` parameters ;
#'
...
#' @param transfo named list of transformation for `Psi_pop` and `psi_pop` parameters ; if NULL id transformation is used;
...
#'
#' theta=list(Psi_pop = Psi_pop,psi_pop = psi_pop,Beta = Beta, beta = beta)
...
indParm <- function(theta,
  ...
  ## Compute Psi parameters -> parameters with no r.e
  for(p in Psi.names){
    trans = transfo[[p]]
    if(is.null(trans)){
    ...
    if(is.null(theta$Beta[[p]])){
      Psi_i[[p]] <- trans.inv(trans(theta$Psi_pop[[p]]))
    }else{
      Psi_i[[p]] <- trans.inv(trans(theta$Psi_pop)+as.numeric(covariates%*%matrix(theta$Beta[[p]],ncol=1)))
    }
  }
  ...
  return(list(Psi_i=Psi_i,psi_i=psi_i))
}
